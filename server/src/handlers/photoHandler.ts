...
function parseCaptions(text: string) {
  const result: Record<string, string> = { ig: '', fb: '', wa: '' };
...
export function setupPhotoHandler(bot: Telegraf<BotContext>) {
  const lastGenerated: Record<number, { imageUrl: string, captions: Record<string, string> }> = {};
  ...
  bot.action('publish_ig', async (ctx) => {
    ...
    if (!data) return ctx.answerCbQuery("❌ Ошибка.");
