...
export const contentScene = new Scenes.BaseScene(CONTENT_SCENE);
contentScene.enter(async (ctx) => {
    await ctx.replyWithHTML('📺 <b>ברוכים הבוס לסטודיו התוכן!</b>\n\n' +
        ...
        '⬅️ <b>חזרה</b> — חזרה לתפריט הראשי.', Markup.inlineKeyboard([
...
contentScene.on('text', async (ctx) => {
    if (ctx.session.state !== 'WAITING_FOR_TOPIC')
        return;
