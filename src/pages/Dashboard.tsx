...
const Dashboard = () => {
  ...
  // Guard: if user data is missing, we don't crash, we use defaults or placeholders
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const enhanceResponse = await fetch(`${baseUrl}/api/enhance`, {
        ...
        body: JSON.stringify({
          image: imagePreview,
          studioName: safeUser.first_name || 'Beauty Master',
          address: 'BeautyOS AI Studio'
        }),
      });

      if (!enhanceResponse.ok) throw new Error('Enhance API error');
      const enhanceData = await enhanceResponse.json();
      
      // Use the first enhanced image for the preview if available
      if (enhanceData.enhancedImages?.[0]) {
        setImagePreview(enhanceData.enhancedImages[0]);
      }

      // Step 2: Analyze and generate captions
      const analyzeResponse = await fetch(`${baseUrl}/api/analyze`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          image: enhanceData.enhancedImages?.[0] || imagePreview,
          masterName: safeUser.first_name || 'Beauty Master'
        }),
      });
